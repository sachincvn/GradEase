...
abstract interface class AuthRemoteDataSource {
  ...
  Future<AuthLoginModel?> getStudentDetail(String email);
  Future<AuthLoginModel> registerStudent(
    String fullName,
    String fatherName,
    DateTime dob,
    String gender,
...
class AuthRemoteDataSourceImpl extends GradEaseRestService
  ...
  @override
  Future<AuthLoginModel?> getStudentDetail(String email) async {
    final restRequest = createGetRequest(RestResources.getStudentDetail(email));
    final response = await executeRequest(restRequest);
    return RestResponse<AuthLoginModel>.fromJson(
      response.data,
      fromJson: (data) => AuthLoginModel.fromMap(data),
    ).data;
  }
